...
class WaterPayBill extends StatefulWidget {
  String water_company;
  String server_mtrno;

  WaterPayBill(this.water_company, this.server_mtrno);

  @override
  _WaterPayBillState createState() =>
      _WaterPayBillState(this.water_company, this.server_mtrno);
}

class _WaterPayBillState extends State<WaterPayBill> {
  // access preferences
  String userid;
  ...
  String water_company, meter_no, amount, pmt_mode;
  String server_mtrno;
  ...
  Widget build(BuildContext context) {
    if (water_company == "Nairobi Water") {
      server_mtrno = nai_meter_s;
      // minamt = 500;
    }
    if (water_company == "Kisumu Water") {
      server_mtrno = kiwasco_s;
    }
    ...
    Future validate() async {
      ...
      } else if (meter_no?.isEmpty ?? false) {
        return showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
              ...
              switch (pmt_mode) {
                case "MPESA":
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => Mpesa(sid, amountBilled, acc_no,
                              oid, '0', payto, paygrp, accno)));
                  break;
                case "AIRTELMONEY":
                  Navigator.push(
                      context,
